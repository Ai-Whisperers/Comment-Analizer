...
# Consolidated analysis - no more stub dependencies
...
class AIAnalysisAdapter:
    ...
    def __init__(self):
        ...
        # Consolidated fallback analyzers - simplified
        self.enhanced_analyzer = None  # Will use integrated fallback logic
        self.improved_analyzer = None  # Will use integrated fallback logic
    ...
    def process_uploaded_file_with_ai(self, uploaded_file) -> Optional[Dict]:
        """
        Main entry point - consolidated AI analysis implementation
        """
        logger.info("🚀 Using consolidated AI adapter implementation")
    ...
    def _convert_ai_results_to_expected_format(self, ai_results: List[Dict], comments: List[str], 
        ...
        if has_nps and nps_data:
            ...
            for result in enhanced_results:
                ...
                # Inline NPS calculation (simplified)
                if sentiment == 'positivo':
                    nps_score = 8 + (intensity * 0.5)  # 8-8.5 range
                elif sentiment == 'negativo':
                    nps_score = 3 + (intensity * 0.3)  # 3-3.3 range
                else:
                    nps_score = 6 + (intensity * 0.2)  # 6-6.2 range
                nps_score = max(0, min(10, nps_score))  # Clamp to 0-10
